# 前沿多模态模型开发与应用实战第二期：Wan2.1多模态生成大模型算法解析与功能抢先体验

多模态生成大模型是一类能够同时创造和合成多种数据形式（如文本📝、图像🎨、音频🎵、视频🎬等）的人工智能系统。这类模型基于前沿的生成式深度学习架构（如扩散模型、GANs等），不仅能理解跨模态信息，还能实现多形式内容的高质量生成与融合！相比单一模态生成技术，多模态生成模型突破了数据形态的界限，通过跨模态对齐与协同创作，可生成更丰富、连贯且符合场景逻辑的内容，极大拓展了创造性AI的应用边界。🚀典型应用包括：图文协同创作、虚拟场景合成、跨模态艺术设计、个性化内容生成（如AI广告、影视特效、交互式叙事）等。随着多模态对齐技术、生成质量和可控性的持续提升，这类模型正在重塑内容生产范式，为创意产业和智能化工具的发展注入全新动能！🔥

本文将介绍Wan2.1模型，涵盖文生视频，图片生视频等方面。内容包括模型细节、训练流程、模型能力的展示，并以PaddleMIX中Wan2.1的实现为例，对代码进行逐步解读。

## 一、引言
自OpenAI推出Sora以来，视频生成技术引起了业界和学术界的广泛关注，推动了该领域的快速发展。能够生成与专业制作内容相媲美的视频的模型的出现，大大提高了内容创作的效率，同时降低了视频制作的成本。

Wan的核心设计灵感来源于扩散变换器（DiT）与流匹配相结合的成功框架，该框架已在文本到图像（T2I）和文本到视频（T2V）任务中证明了通过扩展可实现显著的性能提升。在这一架构范式中，采用交叉注意力来嵌入文本条件，同时精心优化模型设计以确保计算效率和精确的文本可控性。为了进一步增强模型捕捉复杂动态的能力，还融入了完整的时空注意力机制。

Wan提供了两个功能强大的模型，即1.3B和14B参数模型，分别用于提高效率和效果。它还涵盖了多个下游应用，包括图像到视频、指令引导的视频编辑和个人视频生成，涵盖多达八个任务。同时，Wan是首个能够生成中英文视觉文本的模型，显著提升了其实用价值。

## 二、方法
### 2.1 Qwen2-VL
<div align="center">
    <img src="https://github.com/Chenhqi/first-demo/blob/main/image.png" alt="Image 2" style="width: 100%;">
